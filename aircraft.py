...
def calc_distance(path1, path2, time):
...
class AircraftDistributed(object):
    ...
    def find_individual_solution(self):
        return a_star(self.my_map, self.start, self.goal, self.heuristics, self.id, [])
            ...
            if self.id != j:
                if calc_distance(radar_loc[self.id], radar_loc[j], 0) > radar_range:
                    ...
                    if coll[0] == None:
                        ...
                        print("agent1: ",self.id,"agent2: ", j,  ' future collision should be resolved')
